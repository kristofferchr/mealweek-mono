...
export class ApiClient {
  ...
  grpcNewPlannedMeal(mealId: bigint, date: dayjs.Dayjs) {
    return this.withAuth((token) =>
        ApiClient.handleRpcRequest(()=> {
          ...
          return this.plannedMealsService.createPlannedMeal({mealId: mealId, date: timestamp}, {meta: {Authorization: `Bearer ${token}`}}).response
        })
    )
  }
  ...
  grpcGetPlannedMeals(from: dayjs.Dayjs, to: dayjs.Dayjs ) {
    return this.withAuth((token) =>
        ApiClient.handleRpcRequest(() => {
          return this.plannedMealsService.getPlannedMeals({from: Timestamp.fromDate(from.toDate()), to: Timestamp.fromDate(to.toDate())}, {meta: {Authorization: `Bearer ${token}`}}).response
        })
    )
  }
